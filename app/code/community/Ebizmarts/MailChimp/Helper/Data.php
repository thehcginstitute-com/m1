...
class Ebizmarts_MailChimp_Helper_Data extends Mage_Core_Helper_Abstract {
	 ...
	 * @throws Mage_Core_Exception
	 */
	function clearErrorGrid($scopeId, $scope, $excludeSubscribers = false, $filters = null)
	{
	 ...
	 * @throws Mage_Core_Exception
	 */
	function saveLastItemsSent($scopeId, $scope, $filters = null)
	{
		$mailchimpStoreId = $this->getMCStoreId($scopeId, $scope);
	 ...
	 * @param $scope
	 */
	function saveMailchimpConfig(array $vv, $scopeId, $scope, bool $cleanCache = true):void {
		$cfg = Mage::getConfig(); /** @var Cfg $cfg */
		foreach ($vv as $v) {/** @var array $v */
			$cfg->saveConfig($v[0], $v[1], $scope, $scopeId);
		}
		if ($cleanCache) {
			$cfg->cleanCache();
		}
