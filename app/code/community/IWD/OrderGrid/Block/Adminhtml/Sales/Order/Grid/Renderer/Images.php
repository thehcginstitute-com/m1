...
class IWD_OrderGrid_Block_Adminhtml_Sales_Order_Grid_Renderer_Images extends IWD_OrderGrid_Block_Adminhtml_Sales_Order_Grid_Renderer_Abstract
{
    protected function Grid()
    {
        ...
        $helper = Mage::helper('iwd_ordergrid/image');
        $count = 0;
        ...
        foreach ($orderItemCollection as $item) {
            try {
                ...
                try {
                    ...
                    $urlSmallImage = Mage::getDesign()->getSkinUrl(Mage::helper('iwd_ordergrid/image')->getPlaceholder(), array('_area' => 'frontend'));
                }

                try {
                    $urlBigImage = $helper->init($product, 'image')->resize(200);
                } catch (Exception $e) {
                    $urlBigImage = Mage::getDesign()->getSkinUrl(Mage::helper('iwd_ordergrid/image')->getPlaceholder(), array('_area' => 'frontend'));
                }
        ...
        $cell .= $count > 3 ? sprintf('</div><a class="iwd_order_grid_more show row-%s" href="javascript:void(0);" data-row-id="%s" title="%s"></a>',
            $orderId, $orderId, Mage::helper('iwd_ordergrid')->__('Show/hide'))
            : '</div>';
