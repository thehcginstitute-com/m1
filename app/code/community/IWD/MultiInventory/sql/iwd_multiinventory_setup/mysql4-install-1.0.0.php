...
try {
    $installer->run(
        "CREATE TABLE IF NOT EXISTS {$this->getTable('iwd_cataloginventory_stock_address')}(
    ...
    Mage::log($e->getMessage(), null, 'iwd_muliinventory_install.log');
}

/*************** iwd_cataloginventory_stock_store ***************/
try {
    $installer->run(
        "CREATE TABLE IF NOT EXISTS {$this->getTable('iwd_cataloginventory_stock_store')}(
...
try {
    $installer->run(
        "CREATE TABLE IF NOT EXISTS {$this->getTable('iwd_cataloginventory_stock_order_item')}(
            `stock_id` smallint(6) NOT NULL COMMENT 'Page ID',
...
try {
    $installer->run(
        "CREATE TABLE IF NOT EXISTS {$this->getTable('iwd_cataloginventory_stock_order')}(
