...
/**
 * Class IWD_MultiInventory_Model_Cataloginventory_Stock
 */
class IWD_MultiInventory_Model_Cataloginventory_Stock extends Mage_CatalogInventory_Model_Stock
{
    protected function _construct()
    {
        $this->_init('iwd_multiinventory/cataloginventory_stock');
    }
    ...
    protected function isMultiInventoryEnabled()
    {
        ...
        $excludedControllers = array(
            'catalog_product',
            'cataloginventory_order',
            'cataloginventory_stock',
            'cataloginventory_product',
            'system_config'
        );

        return Mage::helper('iwd_multiinventory')->isMultiInventoryEnable()
        && (in_array($module, array('Mage_Adminhtml', 'IWD_MultiInventory_Adminhtml')) && in_array($controller, $excludedControllers));
    }
    ...
    {
        $stockAddress = Mage::getModel('iwd_multiinventory/cataloginventory_stock_address')->getCollection()
            ->addFieldToFilter('stock_id', $this->getStockId());

        if ($stockAddress->getSize() > 0) {
            $stockAddress = $stockAddress->getFirstItem();
        } else {
            $stockAddress = Mage::getModel('iwd_multiinventory/cataloginventory_stock_address');
        }
