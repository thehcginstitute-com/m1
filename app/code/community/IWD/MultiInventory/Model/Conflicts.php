...
/**
 * Class IWD_MultiInventory_Model_Conflicts
 */
class IWD_MultiInventory_Model_Conflicts extends Mage_Core_Model_Abstract
{
    /**
     * @var array
     */
    ...
    {
        return array(
            'model' => Mage::helper('iwd_multiinventory')->__('Model'),
            'block' => Mage::helper('iwd_multiinventory')->__('Block'),
            'helper' => Mage::helper('iwd_multiinventory')->__('Helper'),
        );
    }
            ...
            foreach ($rewritesModules as $base => $rewrites) {
                if (count($rewrites) > 1) {
                    foreach ($rewrites as $class) {
                        $pos = strpos($class, 'IWD_MultiInventory');
                        if ($pos === 0) {
