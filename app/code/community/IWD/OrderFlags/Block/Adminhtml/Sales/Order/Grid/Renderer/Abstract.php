...
/**
 * Class IWD_OrderFlags_Block_Adminhtml_Sales_Order_Grid_Renderer_Abstract
 */
abstract class IWD_OrderFlags_Block_Adminhtml_Sales_Order_Grid_Renderer_Abstract extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    protected $row;
    ...
    public function render(Varien_Object $row)
    {
        ...
        $export = Mage::helper('iwd_orderflags')->isGridExport();
        if ($export) {
            return $this->Export();
        }
    ...
    protected function formatBigData(array $data)
    {
        ...
        $result .= ($count > 4) ? sprintf('</ul><a class="iwd_order_grid_more show row-%s" href="javascript:void(0);" data-row-id="%s" title="%s"></a>',
            $id, $id, Mage::helper('iwd_orderflags')->__('Show/hide'))
            : '</ul></div>';
