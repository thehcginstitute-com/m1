...
IWD.OrderGrid = {
    ...
    init: function(){
        ...
        if (self.isFixGridHeader) {
            ...
            setTimeout(function() {$ji(window).trigger('resize');}, 2000);
        }
    ...
    getGridId: function(gridId) {
        if (gridId == 'row_iwd_ordergrid_grid_order_columns') {
            return 'sales_order_grid_table';
        } else if (gridId == 'row_iwd_ordergrid_customer_orders_orders_grid_columns') {
            return 'customer_orders_grid_table';
        } else if (gridId == 'row_iwd_ordergrid_customer_orders_resent_orders_grid_columns') {
    ...
    colorGridRow: function () {
        ...
        var statusColorsArray = unserialize(IWD.OrderGrid.statusColors);
    ...
    setAlpha: function(item, newAlpha) {
        if ($ji(item).attr('style') == undefined) {
            return;
        }
    ...
    initComplexFilterInput: function() {
        ...
        $ji(document).on('focus', inputs, function () {
            if ($ji(this).width() < 190) {
                ...
                if(typeof($ji('#sales_order_grid').offset()) !== 'undefined'){
                ...
                if(typeof($ji('#sales_order_archive_grid').offset()) !== 'undefined'){
                    if ($ji(this).closest('div').offset().left + 200 > $ji('#sales_order_archive_grid').offset().left + $ji('#sales_order_archive_grid').width()) {
                        $ji(this).closest('div').css('right', '0');
                    }
                }
            }
