...
IWD.MultiInventory.Stock = {
    ...
    init: function()
    {
        ...
        $ji('#iwd_om_stocks_popup').on('hide.bs.modal', function () {
            if($ji('input[name="reload"]').val() == 1 || IWD.MultiInventory.Stock.reloadAfterAssign){
                self.ShowLoadingMask();

                if (typeof IWD.MultiInventory.Stock.reloadAfterAssign === 'string') {
                    document.location.href = IWD.MultiInventory.Stock.reloadAfterAssign;
                } else {
                    location.reload();
                }
            }
    ...
    showAssignStockForm: function(orderId)
    {
        ...
        IWD.MultiInventory.Stock.changed = false;
    ...
    initProductMarkers:function()
    {
        ...
        $ji('#stock_ordered_items_form tbody tr td:nth-child(2) input').each(function(){
            ...
            IWD.MultiInventory.Stock.isAllStocksAssigned = IWD.MultiInventory.Stock.isAllStocksAssigned ? (orderedQty == assignedQty) : IWD.MultiInventory.Stock.isAllStocksAssigned;
    ...
    assignStock: function(orderId) {
        ...
        this.postRequest(formData, this.iwdAssignStockUrl, function(result){
            if(result.reload){
                ...
                $ji(".inventory_order_item_" + orderId).html($ji(result.info).html());
    ...
    updateStockQty:function(item, value)
    {
        ...
        IWD.MultiInventory.Stock.changed = IWD.MultiInventory.Stock.changed ? IWD.MultiInventory.Stock.changed : (value != preValue);
...
IWD.MultiInventory.Stock.Order = {
    init: function()
    {
        IWD.MultiInventory.Stock.init();
    },

    assignStock: function(orderId) {
        var self =  IWD.MultiInventory.Stock;
        var url = IWD.MultiInventory.Stock.Order.iwdAssignStockUrl;
        ...
        self.postRequest(formData, url, function(result){
            if(result.reload){
                ...
                $ji.each(result.info, function (i, item) {
                    $ji(".inventory_order_item_" + i).html($ji(item).html());
                });
                self.closeAssignStockForm(orderId);
                $ji('#iwd_om_stocks_popup').modaliwd('hide');
            }
    ...
    showAssignStockForm: function(orderId)
    {
        ...
        IWD.MultiInventory.Stock.changed = false;
        IWD.MultiInventory.Stock.postRequest(data, IWD.MultiInventory.Stock.Order.iwdAssignStockFormUrl, function(result){
            IWD.MultiInventory.Stock.Order.displayAssignStockForm(result.form);
        });
    },
    ...
    displayAssignStockForm: function(form)
    {
        ...
        IWD.MultiInventory.Stock.initProductMarkers();

        if (IWD.MultiInventory.Stock.isAllStocksAssigned && typeof IWD.MultiInventory.Stock.reloadAfterAssign === 'string') {
            document.location.href = IWD.MultiInventory.Stock.reloadAfterAssign;
        } else {
            IWD.MultiInventory.Stock.initNiceScroll();
            IWD.MultiInventory.Stock.showPopup();
        }
    }
};